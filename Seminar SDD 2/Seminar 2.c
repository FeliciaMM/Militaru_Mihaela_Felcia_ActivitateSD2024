...
struct Calculator* copiazaNCalculatoare(struct Calculator* calculatoare, int nrCalculatoare, int nrCalulcatoareCopiate) {
	struct Calculator* copiate = (struct Calculator*)malloc(sizeof(struct Calculator) * nrCalulcatoareCopiate);
	for (int i =0; i < nrCalulcatoareCopiate; i++) {
...
struct Calculator* primulCalculatorCuPret(struct Calculator* calculatoare, int nrCalculatoare) {
	for (int i = 0; i < nrCalculatoare; i++) {
		if (calculatoare[i].pret > 0) {
			return &calculatoare[i];
		}
	}
	
	return &calculatoare[0];
}
...
void main() {
	...
	printf("\n\n");
	struct Calculator* primul = primulCalculatorCuPret(calculatoare, nrCalculatoare);
	afisareCalculator(*primul);
